...
## 3b. Third-Party Source & Assets Vendored Into The Repository

The first third-party files ever copied into this repository. Everything else
in the register is original work generated by this project.

**Package:** `@designcodeio/threeui@1.2.0` — **MIT**, Copyright (c) 2026 Meng To.
Published by `mengto`, the same owner as `github.com/MengTo/kage` and
`threeui.com`. MIT permits use, copying, modification and redistribution
provided the copyright and permission notice travel with the files.

**Extracted by:** `scripts/sync-kage.mjs` (re-run after any package upgrade).
**Notice kept at:** `public/landing-pages/ATTRIBUTION.txt` — do not delete it.

| Path | Origin | Licence | Notes |
|---|---|---|---|
| `public/landing-pages/kage.html` | `@designcodeio/threeui` → `lib-dist/assets/landing-pages/kage.html` | MIT (Meng To) | The authored Kage single-file experience, 4,821 lines. Verified byte-identical to `MengTo/kage`'s `index.html` after normalising line endings. |
| `public/landing-pages/secret-pathways-assets/generated/*.webp` | same package | MIT (Meng To) | 4 cinematic scene plates. Byte-identical to the upstream repository's copies. |
| `public/landing-pages/secret-pathways-assets/foreground/png/*.webp` | same package | MIT (Meng To) | 10 alpha-preserving foreground cutouts. |
| `public/landing-pages/secret-pathways-assets/fonts.css` | same package | MIT (Meng To) | Onest, base64-embedded WOFF2. **No remote font requests.** |
| `public/landing-pages/secret-pathways-assets/three.min.js` | same package | **MIT (three.js project)** | A vendored three.js r149 build carrying its own notice inside the file. Separate from, and not superseded by, the Meng To copyright above. |

### Standing conditions

1. **The notice must ship.** `ATTRIBUTION.txt` sits beside the files and is
   copied into `dist/` by the build. Removing it would breach the licence.
2. **These files are third-party, not FandomVerse work.** They depict a
   fictional Kyoto mountain temple and contain none of this project's original
   fiction. They must never be described as FandomVerse assets, nor counted in
   the 161-asset procedural register in §1.
3. **Do not hand-edit.** `scripts/sync-kage.mjs` overwrites the folder. Any
   adaptation should be a deliberate, separately-recorded fork — permitted by
   MIT, but it ends the byte-identical provenance claim above and the
   attribution requirement still applies to the derivative.
4. The images inside were generated with GPT Image 2 and art-directed by the
   upstream author, per that project's own attribution statement. This project
   makes no independent claim about their ownership status.
...
**Third-party vendored files (§3b) are tracked separately** and are the only non-original files in the repository. They are MIT-licensed with the notice retained, are not part of the FandomVerse asset register, and are served only at `/landing-pages/kage.html` — no FandomVerse route requests them.
