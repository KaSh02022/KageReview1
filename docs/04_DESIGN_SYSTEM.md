...
**Phase 5 — how the accent is actually applied.** All seven hubs render from one component with no per-category branching (`02_PRODUCT_ARCHITECTURE.md` §16). The accent reaches the UI through exactly three paths: `Card`'s `accent` prop (hover/focus edge treatment), the hub hero's `--hero-accent` custom property (eyebrow + tagline), and the generated art, whose gradients are built from the same hex values as the tokens. No component reads a category id to decide how to look.

Each category also carries a `visualMotif` string (e.g. Anime — "Ember gradients and blade-light streaks", Manga — "Screentone monochrome with a single accent"), rendered as the hero eyebrow. This is deliberate: it gives each world a stated identity **in text**, which both reinforces the art direction and satisfies the accessibility requirement that category identity never be communicated by colour alone — verified by an E2E assertion, not by inspection.

**Contrast fix (D-039).** `Badge tone="primary"` used `--color-primary` on its own tint at 3.94:1, failing WCAG AA for small text. Phase 5 added `--color-primary-on-tint` (#8da4ff) for text on tinted primary surfaces. Use it — not `--color-primary` — whenever primary-coloured text sits on a primary-tinted background.
