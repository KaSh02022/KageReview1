...
| AI | Role | Scope |
|---|---|---|
| Claude (Sonnet 5 / Opus 5, per session) | Primary implementation agent | Repository setup, architecture, React/TypeScript/JavaScript code, Three.js/React Three Fiber, application logic, tests, debugging, optimization, documentation, build, release |
...
| Gemini | Visual asset generation | Images and visual exploration. **Not yet used — no image has been generated.** The production specification is `AI_IMAGE_ASSET_BIBLE.md` + `GEMINI_IMAGE_PROMPTS.md`; the acceptance gate is `ASSET_PROVENANCE.md` + `AI_ASSET_REVIEW_CHECKLIST.md` |
...
| 2026-09-23 | Claude (Opus 5) | Phase 5: **authored the entire content dataset as original fiction** — 7 flagship franchises, 35 character profiles, 21 articles, 21 events, 21 releases, 14 trailers, 14 merchandise items, 28 gallery pieces (D-037); wrote `scripts/generate-content.mjs` to generate both the JSON and all 161 procedural SVG assets (D-036); rebuilt the category hub and all four detail pages; wrote the content-validation gate and hub E2E suite; performed real-screenshot visual QA across 12 pages × 2 viewports | `scripts/generate-content.mjs`, `src/data/**`, `public/assets/generated/**` (161 files), `src/pages/**`, `e2e/category-hubs.spec.ts`, `docs/01`/`02`/`05`/`06`/`07`/`08`/`09`/`11` updates | All prose is AI-authored original fiction, reviewed to confirm it makes **no factual claims about real people, groups, productions, or events** — every franchise, character, event and release is invented for this project and labelled as fiction in the data (D-038). Counts and assertions in the completion report are real command output. |
| 2026-09-24 | Claude (Opus 5) | Phase 5B: authored the visual asset pipeline specification — global visual language, seven category visual identities, three measured image classes, A/B/C/D asset classification, 70 production-ready Gemini prompts (generated from live content data), provenance workflow and acceptance checklist. **No image was generated.** | `docs/AI_IMAGE_ASSET_BIBLE.md`, `docs/GEMINI_IMAGE_PROMPTS.md`, `docs/ASSET_MANIFEST.md`, `docs/ASSET_PROVENANCE.md`, `docs/AI_ASSET_REVIEW_CHECKLIST.md`, `scripts/generate-asset-plan.mjs`, `scripts/validate-asset-manifest.mjs`, `src/data/assetManifest.test.ts` | Image classes were derived from real measured render sizes rather than convention, which caught a constraint that would otherwise have ruined every hero (D-044). The manifest validator was confirmed to actually fail on drift before being trusted. |

**Note on the planned Gemini pass:** no image has been generated as of this entry. `ASSET_PROVENANCE.md` §1 states explicitly that generated output is **not** assumed copyright-free, and forbids recording any accepted asset as public domain, royalty-free or owned without verified evidence (D-045).
...
- **AI-004 (AI-generated images permitted):** any Gemini-generated visual asset is logged in `GEMINI_IMAGE_PROMPTS.md` (prompt), `07_AI_USAGE.md` (generation event) and `08_LICENSES.md` (license/ownership status) at the time it is generated, following `ASSET_PROVENANCE.md`. Generated output is never recorded as copyright-free without verified evidence (D-045).
